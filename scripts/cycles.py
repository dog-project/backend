...
def main(conn):
                ...
                WITH wins AS (
                    ...
                    FROM
                      (SELECT
                       ...
                       FROM votes
                       WHERE result = 'loss'
                       AND submission_time < TO_TIMESTAMP('2019-04-9 23:30:20','YYYY-MM-DD HH24:MI:SS')) AS win
                    GROUP BY dog1, dog2
                ), losses AS (
                    SELECT
                      dog1,
                    ...
                    FROM
                      (SELECT
                       ...
                       FROM votes
                       WHERE result = 'win'
                       AND submission_time < TO_TIMESTAMP('2019-04-9 23:30:20','YYYY-MM-DD HH24:MI:SS'))AS loss
                    GROUP BY dog1, dog2
                ), ties AS (
                    SELECT
                      dog1,
    ...
    for cycle, color in zip(cycles,
                            ['black', 'grey', 'red', 'green', 'blue', 'purple', 'pink', 'orange', 'brown']):
